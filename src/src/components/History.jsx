...
const History = ({ showAllOrders,historyUpdate}) => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const actualHistory = await getOrders(showAllOrders); // Pasar el valor `showAllOrders` como parámetro
      ...
      {queryHistory ? (
        history.length > 0 ? (
          ...
          <h4 className="bg-gray-100 p-4 rounded-lg shadow-md mt-10 text-gray-600">
            {showAllOrders ? "No existen órdenes anteriores" : "No tienes órdenes anteriores"}
          </h4>
        )
      ) : null}
    </div>
